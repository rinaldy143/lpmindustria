...
class colornews_featured_post_style_two_reverse_widget extends WP_Widget {

	function __construct() {
		$widget_ops  = array(
			'classname'   => 'widget_featured_posts_style_two_reverse colornews_custom_widget',
			'description' => __( 'Display latest posts or posts of specific category in reverse.', 'colornews' ),
		);
		$control_ops = array(
			'width'  => 200,
			'height' => 250,
		);
		parent::__construct( false, $name = __( 'TG: Featured Reverse Posts (Style 2)', 'colornews' ), $widget_ops );
	}
			...
			<div class="tg-block-wrapper clearfix">
				...
				<div class="featured-post-wrapper clearfix">
					<div class="tg-column-wrapper">
						...
						$i = 1;
						$first_post_html = '';
						$following_post_html = '<div class="tg-column-2 following-post">';
						...
						while ( $get_featured_posts->have_posts() ) :
							...
							<?php
							$single_article_html = ob_get_clean(); // Get the buffered content and clean the buffer
	
							if ( $i == 1 ) {
								$first_post_html .= '<div class="tg-column-2 first-post">' . $single_article_html . '</div>';
							} else {
								$following_post_html .= $single_article_html;
							}
	
							$i++;
						endwhile;
	
						$following_post_html .= '</div>';
	
						echo $following_post_html; // Display following posts on the left
						echo $first_post_html; // Display first post on the right
